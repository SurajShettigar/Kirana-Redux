                                           ...
                                           const void *data_buffer, vk::Buffer *out_buffer) const
{
    ...
    if (data_buffer && !writeBuffer(encoder, id, *out_buffer, main_create_info.size, data_buffer))
    {
...
bool MemoryAllocator::writeBuffer(const CommandEncoder &encoder, const AllocationID id, const vk::Buffer buffer,
                                  const uint64_t size, const void *data) const
{
    if (!m_allocations.contains(id) || size == 0 || data == nullptr)
    {
        return false;
    }

    const auto &allocation = m_allocations.at(id);
    ...
    if (memory_flags & vk::MemoryPropertyFlagBits::eHostVisible)
    {
        ...
        if (const auto result = vmaCopyMemoryToAllocation(m_handle, data, allocation.handle, 0, size);
            result != VK_SUCCESS)
        {
            core::Logger::error(LOG_CHANNEL_VULKAN,
    ...
    else
    {
        ...
        auto barrier = vk::BufferMemoryBarrier2{vk::PipelineStageFlagBits2::eHost,
                                                ...
                                                vk::QueueFamilyIgnored, s_buffer, 0, size};
        encoder.getNativeHandle().pipelineBarrier2(vk::DependencyInfo{vk::DependencyFlags{}, {}, {barrier}});

        const std::vector copy_regions = {vk::BufferCopy2{0, 0, size}};
        encoder.getNativeHandle().copyBuffer2(vk::CopyBufferInfo2{s_buffer, buffer, copy_regions});
